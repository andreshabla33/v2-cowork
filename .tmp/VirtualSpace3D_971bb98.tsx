...
const TELEPORT_DISTANCE = 15; // Distancia 3D para activar teletransportaci├│n

// Sonido de teletransportaci├│n (s├¡ntesis estilo Goku)
const playTeleportSound = () => {
  try {
    const ctx = new AudioContext();
    const now = ctx.currentTime;
    
    // Sweep ascendente (desaparici├│n)
...
const StableVideo: React.FC<StableVideoProps> = ({ stream, muted = false, muteAudio = false, className = '' }) => {
  ...
  useEffect(() => {
    ...
    // Solo actualizar si el stream realmente cambi├│
...
const Avatar: React.FC<AvatarProps> = ({ position, config, name, status, isCurrentUser, animationState = 'idle', direction, reaction, videoStream, camOn, showVideoBubble = true, message, onClickAvatar }) => {
  ...
  // Auto-ocultar el label despu├®s de 2 segundos
  useEffect(() => {
    if (showStatusLabel) {
  ...
  return (
    <group position={position} onClick={(e) => { if (isCurrentUser && onClickAvatar) { e.stopPropagation(); onClickAvatar(); } }}>
        ...
        <Html position={[0, 3.5, 0]} center distanceFactor={12} zIndexRange={[100, 0]}>
          <div className="w-24 h-16 rounded-[12px] overflow-hidden border-[2px] border-[#6366f1] shadow-lg bg-black relative transform transition-all hover:scale-125 flex items-center justify-center">
             {videoStream && videoStream.getVideoTracks().length > 0 ? (
               ...
               /* Placeholder cuando hay c├ímara pero no hay stream (usuario lejos) */
      ...
      {/* Reacci├│n emoji encima del avatar - Animaci├│n 2026 */}
      {reaction && (
...
// ============== AVATAR REMOTO CON INTERPOLACI├ôN ==============
const RemoteAvatarInterpolated: React.FC<{
  ...
  // Actualizar posici├│n destino cuando cambia
  useEffect(() => {
    ...
    if (jumpDist > TELEPORT_DISTANCE * 0.8) {
      // Salto grande detectado ÔåÆ teleport visual
  ...
  // Interpolar suavemente hacia la posici├│n destino
  useFrame(() => {
...
const CameraFollow: React.FC<{ orbitControlsRef: React.MutableRefObject<any> }> = ({ orbitControlsRef }) => {
  ...
  useFrame(() => {
    ...
    // Primera vez: centrar c├ímara en el jugador
    ...
    if (moved) {
      // Mover target y c├ímara juntos (mantiene la rotaci├│n actual)
...
// ============== EFECTO VISUAL TELETRANSPORTACI├ôN ==============
...
const Player: React.FC<PlayerProps> = ({ currentUser, setPosition, stream, showVideoBubble = true, message, orbitControlsRef, reactions = [], onClickAvatar, moveTarget, onReachTarget, teleportTarget, onTeleportDone }) => {
  ...
  useEffect(() => {
    ...
    // Fase 2: Mover al destino despu├®s de 300ms
    teleportTimerRef.current = setTimeout(() => {
      ...
      // Sincronizar posici├│n inmediatamente
  ...
  useFrame((state, delta) => {
    ...
    if (hasKeyboardInput) {
      ...
      // Movimiento autom├ítico hacia el destino (doble clic estilo Gather)
      ...
      if (dist < 0.15) {
        ...
        // Transici├│n walk ÔåÆ run: empieza caminando, despu├®s de 0.4s corre
        ...
        // Determinar direcci├│n visual del avatar (8 direcciones)
        // En la escena: +Z = hacia la c├ímara (front), -Z = alej├índose (up)
        const absX = Math.abs(distX);
        const absZ = Math.abs(distZ);
        const ratio = Math.min(absX, absZ) / Math.max(absX, absZ);
        const isDiagonal = ratio > 0.4; // Si ambos ejes tienen magnitud similar ÔåÆ diagonal
        ...
        // Animaci├│n: walk al inicio, run despu├®s
    ...
    // Detectar si hay movimiento (teclado o autom├ítico)
    ...
    // Actualizar posici├│n para CameraFollow (├║nica fuente de verdad)
  ...
  return (
    <>
      <group ref={groupRef} position={[positionRef.current.x, 0, positionRef.current.z]}>
        ...
        {/* M├║ltiples emojis flotantes estilo Gather */}
        {reactions.map((r, idx) => (
...
const Scene: React.FC<SceneProps> = ({ currentUser, onlineUsers, setPosition, theme, orbitControlsRef, stream, remoteStreams, showVideoBubbles = true, localMessage, remoteMessages, localReactions, remoteReaction, onClickAvatar, moveTarget, onReachTarget, onDoubleClickFloor, teleportTarget, onTeleportDone }) => {
  ...
  return (
    <>
      ...
      {/* C├ímara Perspectiva para rotaci├│n 3D */}
      <PerspectiveCamera
        makeDefault
...
const VideoHUD: React.FC<VideoHUDProps> = ({
  ...
  // Detectar si el usuario local est├í hablando
    ...
    <>
            ...
            <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
              <div className="bg-white/5 backdrop-blur-2xl px-4 py-2 rounded-2xl border border-white/10">
                <span className="text-sm font-medium text-white/90">
                  {expandedId === 'local' ? 'Tu c├ímara' : expandedId === 'screen' ? 'Tu pantalla' : expandedId?.startsWith('screen-') ? `${usersInCall.find(u => u.id === expandedId?.replace('screen-', ''))?.name || 'Usuario'} - Pantalla` : usersInCall.find(u => u.id === expandedId)?.name || 'Usuario'}
                </span>
              </div>
              <button 
      ...
      <div className={`absolute left-1/2 top-24 -translate-x-1/2 pointer-events-auto z-50 transition-all duration-500 ${
        ...
        {usersInCall.map((u) => {
          ...
          // Si el usuario est├í compartiendo pantalla, no mostrar su c├ímara aqu├¡ (se muestra como PiP junto a la pantalla)
            ...
            <div key={u.id} className={`relative bg-zinc-900 rounded-[28px] overflow-hidden shadow-2xl group transition-all duration-300 ${
              ...
              {isWaving && (
                <div className="absolute inset-0 bg-indigo-500/20 flex items-center justify-center z-20 animate-pulse">
                  <span className="text-4xl animate-bounce">­ƒæï</span>
                </div>
              )}
              {/* Prioridad: 1) C├ímara OFF = foto, 2) C├ímara ON + stream = video, 3) C├ímara ON sin stream = conectando */}
              {!u.isCameraOn ? (
                /* Usuario tiene c├ímara apagada - mostrar foto de perfil o inicial */
                ...
                /* Usuario tiene c├ímara ON y hay stream disponible */
                <StableVideo 
                  stream={remoteStream} 
                ...
                /* Usuario tiene c├ímara ON pero stream no disponible a├║n */
                <div className="absolute inset-0 flex items-center justify-center bg-zinc-900">
              ...
              <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
                ...
                <div className="bg-black/60 backdrop-blur-md px-1.5 py-0.5 rounded text-[9px] text-white/70">
                  {distance < 50 ? '­ƒöè' : distance < 100 ? '­ƒöë' : '­ƒöê'}
                </div>
              </div>
              {/* Controles en hover */}
        ...
        {usersInCall.map((u) => {
          ...
          return (
            <React.Fragment key={`screen-group-${u.id}`}>
              ...
              {/* Burbuja de c├ímara peque├▒a (PiP) cuando tambi├®n comparte pantalla */}
              {hasActiveCamera && (
...
const ICE_SERVERS = [
  // STUN servers (gratuitos, solo para descubrir IP p├║blica)
  { urls: 'stun:stun.l.google.com:19302' },
  { urls: 'stun:stun1.l.google.com:19302' },
  { urls: 'stun:stun2.l.google.com:19302' },
  { urls: 'stun:stun3.l.google.com:19302' },
  { urls: 'stun:stun4.l.google.com:19302' },
  // TURN servers de Metered (gratuitos con l├¡mite)
  { 
...
const VirtualSpace3D: React.FC<VirtualSpace3DProps> = ({ theme = 'dark', isGameHubOpen = false, isPlayingGame = false }) => {
  ...
  // Estado de configuraci├│n de c├ímara (compartido entre BottomControlBar y VideoHUD)
  const [cameraSettings, setCameraSettings] = useState<CameraSettings>(loadCameraSettings);
  
  // Estado de configuraci├│n de audio
  ...
  useEffect(() => {
    const cargarCargo = async () => {
      ...
      if (data?.cargo) {
        console.log('­ƒôï Cargo del usuario cargado:', data.cargo);
  ...
  const selectedCameraIdRef = useRef<string>(loadCameraSettings().selectedCameraId); // C├ímara seleccionada por usuario
  ...
  // Ref para tracking de hist├®resis (evitar parpadeo en el l├¡mite de proximidad)
  ...
  const usersInCall = useMemo(() => {
    ...
    const users = onlineUsers.filter(u => {
      ...
      // Validar coordenadas (ignorar 0,0 que suele ser inicializaci├│n)
      ...
      // Esto evita que la conexi├│n oscile cuando se est├í en el borde.
  ...
  useEffect(() => {
    ...
    const checkAudioLevel = () => {
      ...
      // Si el promedio es mayor a 30, el usuario local est├í hablando
      ...
      .on(
        ...
        (payload) => {
          ...
          if (notif.tipo === 'consentimiento_respuesta' && notif.titulo?.includes('Aceptado')) {
            console.log('Ô£à Consentimiento aceptado por el evaluado');
  ...
  // Estado para trigger externo de grabaci├│n
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      // No activar si est├í escribiendo
      const activeEl = document.activeElement;
  ...
  // Atajos de teclado num├®rico 1-8 para emojis r├ípidos
  useEffect(() => {
    const emojiKeys = ['­ƒæì', '­ƒöÑ', 'ÔØñ´©Å', '­ƒæÅ', '­ƒÿé', '­ƒÿ«', '­ƒÜÇ', 'Ô£¿'];
  ...
  const createPeerConnection = useCallback((peerId: string) => {
    ...
    pc.ontrack = (event) => {
      ...
      // Detectar si es screen share por label (m├ís confiable que contador)
      ...
      if (event.track.kind === 'video') {
        // Usar detecci├│n por label como m├®todo principal
    ...
    // Agregar screen share si est├í activo
  ...
  const handleOffer = useCallback(async (offer: RTCSessionDescriptionInit, fromId: string) => {
    // Reutilizar conexi├│n existente si existe (para renegociaciones)
    ...
    if (!pc) {
      ...
      // Si es renegociaci├│n, verificar que los tracks locales est├®n agregados
  ...
  useEffect(() => {
    ...
    // Programar desconexiones para usuarios que ya no est├ín ONLINE
  ...
  // NOTA: La renegociaci├│n autom├ítica cuando el stream cambia fue removida
  // porque causaba conflictos de SDP (m-lines order mismatch).
  // Los tracks se agregan al crear la conexi├│n inicial en createPeerConnection.
  ...
  // Ref para acceder al estado actual dentro de la funci├│n as├¡ncrona
  ...
  // Manejar stream de video - encender/apagar seg├║n proximidad
  useEffect(() => {
    let mounted = true;

    const manageStream = async () => {
      // Si ya est├í procesando, marcar que necesitamos una actualizaci├│n al terminar
      ...
      try {
        ...
        if (shouldHaveStream) {
          if (!activeStreamRef.current) {
            // Usar c├ímara seleccionada si est├í disponible
            ...
            // Configuraci├│n de audio con reducci├│n de ruido y cancelaci├│n de eco
            ...
            console.log('­ƒÄñ Audio constraints:', audioConstraints);
            ...
            const newStream = await navigator.mediaDevices.getUserMedia({ 
              ...
              // Si falla con dispositivos espec├¡ficos, intentar con defaults
            ...
            // Verificar si el estado cambi├│ mientras esper├íbamos
            if (!shouldHaveStreamRef.current) {
              console.log('Stream loaded but no longer needed (state changed), stopping...');
              newStream.getTracks().forEach(t => t.stop());
              return; // Terminar├í en finally y disparar├í pending update si es necesario
            }
            ...
            if (peerConnectionsRef.current.size > 0) {
              ...
              peerConnectionsRef.current.forEach(async (pc, peerId) => {
                // Verificar qu├® tracks ya tiene el peer
                const senders = pc.getSenders();
          ...
          if (activeStreamRef.current) {
            ...
            // Video: DETENER el track completamente si c├ímara OFF (libera hardware)
            ...
            if (!currentUser.isCameraOn && videoTracks.length > 0) {
              ...
              // C├ímara ON pero no hay video track - obtener nuevo stream de video
          ...
          // No hay proximidad ni screen sharing - apagar c├ímara/mic
          if (activeStreamRef.current) {
        ...
        if (mounted) {
          ...
          // Si hubo cambios pendientes mientras proces├íbamos, ejecutar de nuevo
    ...
    // Debounce de 500ms para evitar parpadeos r├ípidos
  ...
  useEffect(() => {
    ...
    console.log('­ƒÄ¿ Updating peer connections with processed video (background effect)');
    ...
    peerConnectionsRef.current.forEach(async (pc, peerId) => {
      ...
      if (videoSender) {
        try {
          ...
          console.log('­ƒÄ¿ Replaced video track for peer', peerId);
        } catch (err) {
  ...
  useEffect(() => {
    if (cameraSettings.backgroundEffect === 'none' && processedStream) {
      ...
      if (originalVideoTrack) {
        console.log('­ƒÄ¿ Restoring original video track to peers');
        peerConnectionsRef.current.forEach(async (pc, peerId) => {
          const videoSender = pc.getSenders().find(s => s.track?.kind === 'video');
          if (videoSender) {
            try {
              await videoSender.replaceTrack(originalVideoTrack);
              console.log('­ƒÄ¿ Restored original video for peer', peerId);
            } catch (err) {
  ...
  // Mantiene el audio estable y cambia a video original cuando la p├ígina est├í oculta
  useEffect(() => {
    ...
    const handleVisibilityChange = async () => {
      if (document.hidden) {
        // P├ígina oculta: crear AudioContext para mantener audio activo
        console.log('­ƒöè Page hidden - activating audio keepalive');
        try {
        ...
        if (processedStream && stream && cameraSettings.backgroundEffect !== 'none') {
          ...
          const originalVideoTrack = stream.getVideoTracks()[0];
          if (originalVideoTrack) {
            console.log('­ƒô╣ Page hidden - switching to original video (avoiding canvas freeze)');
            peerConnectionsRef.current.forEach(async (pc, peerId) => {
              const videoSender = pc.getSenders().find(s => s.track?.kind === 'video');
              if (videoSender) {
                try {
                  await videoSender.replaceTrack(originalVideoTrack);
                  console.log('­ƒô╣ Switched to original video for peer', peerId);
                } catch (err) {
        ...
        // P├ígina visible: limpiar AudioContext
        console.log('­ƒöè Page visible - deactivating audio keepalive');
        if (silentSource) {
        ...
        if (wasUsingProcessedStream && processedStream) {
          ...
          if (processedVideoTrack) {
            console.log('­ƒô╣ Page visible - restoring processed video');
            peerConnectionsRef.current.forEach(async (pc, peerId) => {
              const videoSender = pc.getSenders().find(s => s.track?.kind === 'video');
              if (videoSender) {
                try {
                  await videoSender.replaceTrack(processedVideoTrack);
                  console.log('­ƒô╣ Restored processed video for peer', peerId);
                } catch (err) {
  ...
  // Funci├│n para resetear la vista de la c├ímara
  const handleResetView = useCallback(() => {
    if (orbitControlsRef.current) {
      ...
      // Resetear posici├│n de c├ímara a vista isom├®trica por defecto
  ...
  return (
    <div className="w-full h-full relative bg-black" onClick={handleCanvasClick}>
        ...
        <Suspense fallback={null}>
          <Scene
            ...
            onDoubleClickFloor={(point) => {
              // Calcular distancia desde posici├│n actual del avatar
              const playerX = (currentUser.x || 400) / 16;
              const playerZ = (currentUser.y || 400) / 16;
              ...
              if (dist > TELEPORT_DISTANCE) {
                // Distancia larga ÔåÆ teletransportaci├│n estilo Goku
                setMoveTarget(null);
                setTeleportTarget({ x: point.x, z: point.z });
              } else {
                // Distancia corta ÔåÆ caminar/correr
      ...
      {/* Indicador discreto de grabaci├│n para otros usuarios (no el grabador) */}
      ...
      <button
        ...
        title="Resetear vista (centrar c├ímara en tu avatar)"
      >
      ...
      {/* VideoHUD - solo se muestra cuando hay usuarios cerca (burbuja local ahora est├í en el avatar) */}
      ...
      <BottomControlBar
        ...
        onAudioSettingsChange={async (newSettings) => {
          ...
          // Aplicar cambios de micr├│fono en tiempo real si hay stream activo
          if (activeStreamRef.current && newSettings.selectedMicrophoneId) {
            try {
              console.log('­ƒÄñ Applying new microphone:', newSettings.selectedMicrophoneId);
              ...
              if (oldAudioTrack && newAudioTrack) {
                ...
                peerConnectionsRef.current.forEach(async (pc, peerId) => {
                  ...
                  if (audioSender) {
                    ...
                    console.log('­ƒÄñ Replaced audio track for peer', peerId);
                  }
                });
                
                // Aplicar estado de mute actual
                newAudioTrack.enabled = currentUser.isMicOn;
                console.log('­ƒÄñ New microphone applied successfully');
              }
            } catch (err) {
      ...
      {incomingWave && (
        <div className="fixed top-20 right-4 z-[201] animate-slide-in">
          <div className="bg-amber-500 text-black px-4 py-3 rounded-xl shadow-2xl flex items-center gap-3">
            <span className="text-3xl animate-wave">­ƒæï</span>
            <div>
              <p className="font-bold text-sm">{incomingWave.fromName}</p>
              <p className="text-xs opacity-80">te est├í saludando</p>
            </div>
            <button 
      ...
      {/* Recording Manager V2 con an├ílisis conductual avanzado */}
      {hasActiveCall && (
        <RecordingManager
          ...
          onProcessingComplete={(resultado) => {
            console.log('Ô£à An├ílisis conductual completado:', resultado?.tipo_grabacion, resultado?.analisis);
          }}
          headlessMode={true}
      ...
      <ConsentimientoPendiente
        onConsentimientoRespondido={(grabacionId, acepto) => {
          console.log(`­ƒôØ Consentimiento ${acepto ? 'aceptado' : 'rechazado'} para grabaci├│n:`, grabacionId);
        }}
      />
